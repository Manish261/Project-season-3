# -*- coding: utf-8 -*-
"""
Created on Tue Sep  8 19:07:52 2020
...
def create_app8_ui():
  dropdown_style={'width' : '550px', 'text-align' : 'center', 'vertical-align' : 'center', 'margin-left' : '200px' ,'height':'40px','padding-top': '4px','padding-bottom': '2px','border-radius':'25px'}
  main_layout = html.Div(style = { 'background-color' : 'black' },
    children=  [
  html.H1('Terrorism Analysis with Insights', id='Main_title', style={'textAlign':'center','background-color': 'grey'}),
  html.Img(src="assets/s.webp" ,style="height: 350px; width: 100%;"),
  dcc.Tabs(id="Tabs", value="Map",children=[
      dcc.Tab(label="Map tool" ,id="Map tool",value="Map",style={'textAlign':'center','background-color': 'grey'}, children=[
          dcc.Tabs(id = "subtabs", value = "WorldMap",children = [
              dcc.Tab(label="World Map tool", id="World", value="WorldMap",style={'textAlign':'center','background-color': 'grey'}),
              dcc.Tab(label="India Map tool", id="India", value="IndiaMap",style={'textAlign':'center','background-color': 'grey'})
              ]),
          dcc.Dropdown(
              id='month', 
                options=month_list,
          ...
          dcc.Dropdown(
                ...
                multi = True,
                style=dropdown_style ),
          html.H3('Select the Year', id='year_title',style={ 'height' : '20px', 'text-align' : 'center', 'vertical-align' : 'center', 'width' : '100%','background-color':'white' }),
          dcc.RangeSlider(
                    id='year-slider',
                    min=min(year_list),
                    max=max(year_list),
      ...
      dcc.Tab(label = "Chart Tool", id="chart tool", value="Chart",style={'textAlign':'center','background-color': 'grey'}, children=[
          dcc.Tabs(id = "subtabs2", value = "WorldChart",children = [
              dcc.Tab(label="World Chart tool", id="WorldC", value="WorldChart",style={'textAlign':'center','background-color': 'grey'}),          
            dcc.Tab(label="India Chart tool", id="IndiaC", value="IndiaChart",style={'textAlign':'center','background-color': 'grey'})]),
  ...
  html.Footer("Made by Manish Khyani With Forsk Technologies @ 2020",style={'height': '20px', 'width': '100%','background-color': 'grey','textAlign':'center','padding-top': '8px','padding-bottom': '8px','border-radius':'25px'})
  ])
  return main_layout
...
def update_app9_ui(Tabs, month_value, date_value,region_value,country_value,state_value,city_value,attack_value,year_value,chart_year_selector, chart_dp_value, search,
    ...
    if Tabs == "Map":
        ...
        mapFigure.update_layout(margin={"r":0,"t":25,"l":0,"b":20})
